...
class MftRecord:
    ...
    def parse_record(self) -> None:
        try:
            self.magic = struct.unpack(
                "<I",
                self.raw_record[
                    MFT_RECORD_MAGIC_NUMBER_OFFSET : MFT_RECORD_MAGIC_NUMBER_OFFSET
                    + MFT_RECORD_MAGIC_NUMBER_SIZE
                ],
            )[0]
            self.upd_off = struct.unpack(
                "<H",
                self.raw_record[
                    MFT_RECORD_UPDATE_SEQUENCE_OFFSET : MFT_RECORD_UPDATE_SEQUENCE_OFFSET
                    + MFT_RECORD_UPDATE_SEQUENCE_SIZE
                ],
            )[0]
            self.upd_cnt = struct.unpack(
                "<H",
                self.raw_record[
                    MFT_RECORD_UPDATE_SEQUENCE_SIZE_OFFSET : MFT_RECORD_UPDATE_SEQUENCE_SIZE_OFFSET
                    + MFT_RECORD_UPDATE_SEQUENCE_SIZE_SIZE
                ],
            )[0]
            self.lsn = struct.unpack(
                "<Q",
                self.raw_record[
                    MFT_RECORD_LOGFILE_SEQUENCE_NUMBER_OFFSET : MFT_RECORD_LOGFILE_SEQUENCE_NUMBER_OFFSET
                    + MFT_RECORD_LOGFILE_SEQUENCE_NUMBER_SIZE
                ],
            )[0]
            self.seq = struct.unpack(
                "<H",
                self.raw_record[
                    MFT_RECORD_SEQUENCE_NUMBER_OFFSET : MFT_RECORD_SEQUENCE_NUMBER_OFFSET
                    + MFT_RECORD_SEQUENCE_NUMBER_SIZE
                ],
            )[0]
            self.link = struct.unpack(
                "<H",
                self.raw_record[
                    MFT_RECORD_HARD_LINK_COUNT_OFFSET : MFT_RECORD_HARD_LINK_COUNT_OFFSET
                    + MFT_RECORD_HARD_LINK_COUNT_SIZE
                ],
            )[0]
            self.attr_off = struct.unpack(
                "<H",
                self.raw_record[
                    MFT_RECORD_FIRST_ATTRIBUTE_OFFSET : MFT_RECORD_FIRST_ATTRIBUTE_OFFSET
                    + MFT_RECORD_FIRST_ATTRIBUTE_SIZE
                ],
            )[0]
            self.flags = struct.unpack(
                "<H",
                self.raw_record[
                    MFT_RECORD_FLAGS_OFFSET : MFT_RECORD_FLAGS_OFFSET
                    + MFT_RECORD_FLAGS_SIZE
                ],
            )[0]
            self.size = struct.unpack(
                "<I",
                self.raw_record[
                    MFT_RECORD_USED_SIZE_OFFSET : MFT_RECORD_USED_SIZE_OFFSET
                    + MFT_RECORD_USED_SIZE_SIZE
                ],
            )[0]
            self.alloc_sizef = struct.unpack(
                "<I",
                self.raw_record[
                    MFT_RECORD_ALLOCATED_SIZE_OFFSET : MFT_RECORD_ALLOCATED_SIZE_OFFSET
                    + MFT_RECORD_ALLOCATED_SIZE_SIZE
                ],
            )[0]
            self.base_ref = struct.unpack(
                "<Q",
                self.raw_record[
                    MFT_RECORD_FILE_REFERENCE_OFFSET : MFT_RECORD_FILE_REFERENCE_OFFSET
                    + MFT_RECORD_FILE_REFERENCE_SIZE
                ],
            )[0]
            self.next_attrid = struct.unpack(
                "<H",
                self.raw_record[
                    MFT_RECORD_NEXT_ATTRIBUTE_ID_OFFSET : MFT_RECORD_NEXT_ATTRIBUTE_ID_OFFSET
                    + MFT_RECORD_NEXT_ATTRIBUTE_ID_SIZE
                ],
            )[0]
            self.recordnum = struct.unpack(
                "<I",
                self.raw_record[
                    MFT_RECORD_RECORD_NUMBER_OFFSET : MFT_RECORD_RECORD_NUMBER_OFFSET
                    + MFT_RECORD_RECORD_NUMBER_SIZE
                ],
            )[0]
    ...
    def parse_attributes(self):
        ...
        while offset < len(self.raw_record) - 8:
            try:
                ...
                attr_type = int(
                    struct.unpack("<L", self.raw_record[offset : offset + 4])[0]
                )
                attr_len = int(
                    struct.unpack("<L", self.raw_record[offset + 4 : offset + 8])[0]
                )
    ...
    def parse_si_attribute(self, offset: int) -> None:
        ...
        if len(si_data) >= 32:
            try:
                self.si_times = {
                    "crtime": WindowsTime(
                        struct.unpack("<L", si_data[:4])[0],
                        struct.unpack("<L", si_data[4:8])[0],
                    ),
                    "mtime": WindowsTime(
                        struct.unpack("<L", si_data[8:12])[0],
                        struct.unpack("<L", si_data[12:16])[0],
                    ),
                    "ctime": WindowsTime(
                        struct.unpack("<L", si_data[16:20])[0],
                        struct.unpack("<L", si_data[20:24])[0],
                    ),
                    "atime": WindowsTime(
                        struct.unpack("<L", si_data[24:28])[0],
                        struct.unpack("<L", si_data[28:32])[0],
                    ),
                }
            except struct.error:
                pass
    ...
    def parse_fn_attribute(self, offset: int) -> None:
        ...
        if len(fn_data) >= 64:
            try:
                self.fn_times = {
                    "crtime": WindowsTime(
                        struct.unpack("<L", fn_data[8:12])[0],
                        struct.unpack("<L", fn_data[12:16])[0],
                    ),
                    "mtime": WindowsTime(
                        struct.unpack("<L", fn_data[16:20])[0],
                        struct.unpack("<L", fn_data[20:24])[0],
                    ),
                    "ctime": WindowsTime(
                        struct.unpack("<L", fn_data[24:28])[0],
                        struct.unpack("<L", fn_data[28:32])[0],
                    ),
                    "atime": WindowsTime(
                        struct.unpack("<L", fn_data[32:36])[0],
                        struct.unpack("<L", fn_data[36:40])[0],
                    ),
                }
                self.filesize = struct.unpack("<Q", fn_data[48:56])[0]
                name_len = struct.unpack("B", fn_data[64:65])[0]
                if len(fn_data) >= 66 + name_len * 2:
                    self.filename = fn_data[66 : 66 + name_len * 2].decode(
                        "utf-16-le", errors="replace"
                    )
                self.parent_ref = (
                    struct.unpack("<Q", fn_data[:8])[0] & 0x0000FFFFFFFFFFFF
                )
            except struct.error:
                pass
    ...
    def parse_object_id_attribute(self, offset: int) -> None:
        ...
        if len(obj_id_data) >= 64:
            try:
                ...
                if self.debug:
                    print(
                        f"Error parsing Object ID attribute for record {self.recordnum}"
                    )
    ...
    def parse_attribute_list(self, offset: int) -> None:
        attr_content_offset = (
            offset + struct.unpack("<H", self.raw_record[offset + 20 : offset + 22])[0]
        )
        attr_content_end = (
            offset + struct.unpack("<L", self.raw_record[offset + 4 : offset + 8])[0]
        )

        while attr_content_offset < attr_content_end:
            try:
                attr_type = struct.unpack(
                    "<L", self.raw_record[attr_content_offset : attr_content_offset + 4]
                )[0]
                attr_len = struct.unpack(
                    "<H",
                    self.raw_record[attr_content_offset + 4 : attr_content_offset + 6],
                )[0]
                name_len = struct.unpack(
                    "B",
                    self.raw_record[attr_content_offset + 6 : attr_content_offset + 7],
                )[0]
                name_offset = struct.unpack(
                    "B",
                    self.raw_record[attr_content_offset + 7 : attr_content_offset + 8],
                )[0]

                if name_len > 0:
                    name = self.raw_record[
                        attr_content_offset
                ...
                vcn = struct.unpack(
                    "<Q",
                    self.raw_record[attr_content_offset + 8 : attr_content_offset + 16],
                )[0]
                ref = struct.unpack(
                    "<Q",
                    self.raw_record[
                        attr_content_offset + 16 : attr_content_offset + 24
                    ],
                )[0]

                self.attribute_list.append(
                    {"type": attr_type, "name": name, "vcn": vcn, "reference": ref}
                )
    ...
    def parse_security_descriptor(self, offset: int) -> None:
        ...
        if len(sd_data) >= 20:
            try:
                ...
                if self.debug:
                    print(
                        f"Error parsing Security Descriptor attribute for record {self.recordnum}"
                    )
    ...
    def parse_volume_name(self, offset: int) -> None:
        ...
        try:
            ...
            self.volume_name = vn_data[2 : 2 + name_length * 2].decode(
                "utf-16-le", errors="replace"
            )
        except struct.error:
            if self.debug:
                print(
                    f"Error parsing Volume Name attribute for record {self.recordnum}"
                )
    ...
    def parse_volume_information(self, offset: int) -> None:
        ...
        if len(vi_data) >= 12:
            try:
                ...
                if self.debug:
                    print(
                        f"Error parsing Volume Information attribute for record {self.recordnum}"
                    )
    ...
    def parse_data(self, offset):
        ...
        try:
            ...
            if non_resident_flag == 0:  # Resident
                ...
                content = self.raw_record[
                    offset + content_offset : offset + content_offset + content_size
                ]
            else:  # Non-resident
                start_vcn = struct.unpack("<Q", data_header[16:24])[0]
                last_vcn = struct.unpack(
                    "<Q", self.raw_record[offset + 24 : offset + 32]
                )[0]
    ...
    def parse_index_allocation(self, offset: int) -> None:
        ...
        try:
            ...
            self.index_allocation = {"data_runs_offset": data_runs_offset}
        except struct.error:
            if self.debug:
                print(
                    f"Error parsing Index Allocation attribute for record {self.recordnum}"
                )
    ...
    def parse_reparse_point(self, offset: int) -> None:
        ...
        try:
            ...
            if self.debug:
                print(
                    f"Error parsing Reparse Point attribute for record {self.recordnum}"
                )
    ...
    def parse_ea_information(self, offset: int) -> None:
        ...
        try:
            ...
            self.ea_information = {"ea_size": ea_size, "ea_count": ea_count}
        except struct.error:
            if self.debug:
                print(
                    f"Error parsing EA Information attribute for record {self.recordnum}"
                )
    ...
    def parse_logged_utility_stream(self, offset: int) -> None:
        ...
        try:
            ...
            if self.debug:
                print(
                    f"Error parsing Logged Utility Stream attribute for record {self.recordnum}"
                )
