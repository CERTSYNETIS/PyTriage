...
class BMCContainer:
        ...
        while len(self.bdat) > 0:
            ...
            if self.btype == self.BIN_CONTAINER:
                ...
                t_bmp = self.b_parse_rgb32b(
                    self.bdat[len(t_hdr) : len(t_hdr) + bl].tobytes()
                )
                ...
                if (
                        ...
                        if "22.bmc" in self.fname:
                            ...
                            for b in [1, 2, 4]:
                                if len(self.bdat) < len(t_hdr) + 64 * 64 * b + 8:
                                    ...
                                    unpack(
                                        "<H",
                                        self.bdat[len(t_hdr) + 64 * 64 * b + 8 :][:2],
                                    )[0]
                                    == 64
                                ):
                                    bl = 64 * 64 * b
                            ...
                            if bl == 0:
                                return False
                    o_bmp = b""
                    t_bmp = self.b_uncompress(
                        self.bdat[len(t_hdr) : len(t_hdr) + t_len].tobytes(),
                        bl // (64 * 64),
                    )
                    if len(t_bmp) > 0:
                    ...
                    if cf == 4:
                        t_bmp = self.b_parse_rgb32b(
                            self.bdat[
                                len(t_hdr) : len(t_hdr) + cf * t_width * t_height
                            ].tobytes()
                        )
                        if t_height != 64:
                            old = True
            ...
            if self.cnt != 0 and len(self.bmps) == self.cnt:
                break
        self.logger.info(f"{len(self.bmps)} tiles successfully extracted in the end.")
        return True
    ...
    def b_uncompress(self, data, bbp):
        ...
        while len(data) > 0:
            ...
            if cmd == -1:
                if rl == 1:
                    self.logger.info(
                        "Unexpected end of compressed stream. Skipping tile."
                    )
                elif rl == 2:
                    self.logger.info(
                        f"Unexpected decompression command encountered (0x{sz}). Skipping tile."
                    )
                return b""
            data = data[sz:]
            if cmd in [0x00, 0xF0]:
    ...
    def b_export(self, dname):
        ...
        if self.big:
            ...
            if self.btype == self.BIN_CONTAINER:
                collage_builder = lambda x, a=self, PAD=len(pad), WIDTH=range(
                    w // 64
                ): b"".join(
                    [
                        b"".join(
                ...
                collage_builder = lambda x, a=self, PAD=len(pad), WIDTH=range(
                    ...
                    [
                        b"".join(
                            [
                                a.bmps[64 * x + k][64 * PAD * j : 64 * PAD * (j + 1)]
                                for k in WIDTH
                            ]
                        )
                        for j in range(64)
            ...
            self.b_write(
                Path(dname) / Path(f"{self.fname.name}_collage.bmp"),
